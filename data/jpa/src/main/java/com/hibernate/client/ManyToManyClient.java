...
public class ManyToManyClient {
	public static void main(String[] args) {
		...
		try {
			...
			/*
			 * Movie movie = (Movie) session.get(Movie.class, 1L); Actor actor = (Actor)
			 * session.get(Actor.class, 2L); actor.getMovies().add(movie);
			 */

			// Updating the owner (Movie)
			/*
			 * Movie movie = (Movie) session.get(Movie.class, 1L); Actor actor = (Actor)
			 * session.get(Actor.class, 2L); movie.getActors().add(actor);
			 */
